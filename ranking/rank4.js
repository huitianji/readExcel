...
async function read (file) {
  const binary = XLSX.readFile(__dirname + '/最佳推荐人.xlsx', {
    type: 'binary'
  })
  const json = XLSX.utils.sheet_to_json(binary.Sheets[binary.SheetNames[0]])
    //console.log(json,"===")
  fs.writeFile(path.join(__dirname, '/rankResult4.js'), beautify(JSON.stringify(transformData(json)), { indent_size: 2 }), (err) => {
    if (err) {
      reject(err)
      console.log(err)
    }
