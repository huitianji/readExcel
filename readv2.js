...
async function read (file) {
  ...
  const binary = XLSX.readFile('./最佳代理人.xlsx', {
    type: 'binary'
  })
  const json = XLSX.utils.sheet_to_json(binary.Sheets[binary.SheetNames[0]])
    //console.log(json,"===")
    // console.log(beautify(JSON.stringify(transformData(json))), '///')
  fs.writeFile(path.join(__dirname, 'resultv2.js'), beautify(JSON.stringify(transformData(json)), { indent_size: 2 }), (err) => {
    if (err) {
      reject(err)
      console.log(err)
    }
